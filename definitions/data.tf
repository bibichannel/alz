# Initiative built-in

# https://www.azadvertizer.net/azpolicyinitiativesadvertizer/924bfe3a-762f-40e7-86dd-5c8b95eb09e6.html
data "azurerm_policy_set_definition" "dine_vm_monitoring" {
  name = "924bfe3a-762f-40e7-86dd-5c8b95eb09e6"
}

# https://www.azadvertizer.net/azpolicyinitiativesadvertizer/f5bf694c-cca7-4033-b883-3a23327d5485.html
data "azurerm_policy_set_definition" "dine_vmss_monitoring" {
  name = "f5bf694c-cca7-4033-b883-3a23327d5485"
}

# https://www.azadvertizer.net/azpolicyinitiativesadvertizer/2b00397d-c309-49c4-aa5a-f0b2c5bc6321.html
data "azurerm_policy_set_definition" "dine_vm_hybrid_monitoring" {
  name = "2b00397d-c309-49c4-aa5a-f0b2c5bc6321"
}

# https://www.azadvertizer.net/azpolicyinitiativesadvertizer/92a36f05-ebc9-4bba-9128-b47ad2ea3354.html
data "azurerm_policy_set_definition" "dine_change_tracking_and_inventory_vm" {
  name = "92a36f05-ebc9-4bba-9128-b47ad2ea3354"
}

# https://www.azadvertizer.net/azpolicyinitiativesadvertizer/c4a70814-96be-461c-889f-2b27429120dc.html
data "azurerm_policy_set_definition" "dine_change_tracking_and_inventory_vmss" {
  name = "c4a70814-96be-461c-889f-2b27429120dc"
}

# https://www.azadvertizer.net/azpolicyinitiativesadvertizer/53448c70-089b-4f52-8f38-89196d7f2de1.html
data "azurerm_policy_set_definition" "dine_change_tracking_and_inventory_vm_arc" {
  name = "53448c70-089b-4f52-8f38-89196d7f2de1"
}

# https://www.azadvertizer.net/azpolicyinitiativesadvertizer/de01d381-bae9-4670-8870-786f89f49e26.html
data "azurerm_policy_set_definition" "dine_mdfc_defender_sql_and_ama" {
  name = "de01d381-bae9-4670-8870-786f89f49e26"
}

# https://www.azadvertizer.net/azpolicyinitiativesadvertizer/e20d08c5-6d64-656d-6465-ce9e37fd0ebc.html
data "azurerm_policy_set_definition" "dine_microsoft_defender_for_endpoint_agent" {
  name = "e20d08c5-6d64-656d-6465-ce9e37fd0ebc"
}

# https://www.azadvertizer.net/azpolicyinitiativesadvertizer/77b391e3-2d5d-40c3-83bf-65c846b3c6a3.html
data "azurerm_policy_set_definition" "dine_defender_endpoint_integration_with_defender_cloud" {
  name = "77b391e3-2d5d-40c3-83bf-65c846b3c6a3"
}

# # https://www.azadvertizer.net/azpolicyinitiativesadvertizer/0884adba-2312-4468-abeb-5422caed1038.html
data "azurerm_policy_set_definition" "dine_diagnostic_settings_to_azure_services" {
  name = "0884adba-2312-4468-abeb-5422caed1038"
}

# # https://www.azadvertizer.net/azpolicyinitiativesadvertizer/1f3afdf9-d0c9-4c3d-847f-89da613e70a8.html
data "azurerm_policy_set_definition" "audit_microsoft_cloud_security_benchmark" {
  name = "1f3afdf9-d0c9-4c3d-847f-89da613e70a8"
}

# # https://www.azadvertizer.net/azpolicyinitiativesadvertizer/e77fc0b3-f7e9-4c58-bc13-cb753ed8e46e.html
data "azurerm_policy_set_definition" "dine_advanced_threat_protection_on_open_source_dbs" {
  name = "e77fc0b3-f7e9-4c58-bc13-cb753ed8e46e"
}

# # https://www.azadvertizer.net/azpolicyinitiativesadvertizer/9cb3cc7a-b39b-4b82-bc89-e5a5d9ff7b97.html
data "azurerm_policy_set_definition" "dine_azure_defender_on_sql_servers_and_managed_instances" {
  name = "9cb3cc7a-b39b-4b82-bc89-e5a5d9ff7b97"
}

# https://www.azadvertizer.net/azpolicyinitiativesadvertizer/130fb88f-0fc9-4678-bfe1-31022d71c7d5.html
data "azurerm_policy_set_definition" "audit_resources_should_be_zone_resilient" {
  name = "130fb88f-0fc9-4678-bfe1-31022d71c7d5"
}

# https://www.azadvertizer.net/azpolicyinitiativesadvertizer/06f19060-9e68-4070-92ca-f15cc126059e.html
data "azurerm_policy_set_definition" "manual_cis_microsoft_azure_foundations_benchmark_v2_0_0" {
  name = "06f19060-9e68-4070-92ca-f15cc126059e"
}

# # https://www.azadvertizer.net/azpolicyinitiativesadvertizer/89c6cddc-1c73-4ac1-b19c-54d1a15a42f2.html
data "azurerm_policy_set_definition" "manual_iso_27001_2013" {
  name = "89c6cddc-1c73-4ac1-b19c-54d1a15a42f2"
}

# Definition built-in

# https://www.azadvertizer.net/azpolicyadvertizer/2465583e-4e78-4c15-b6be-a36cbc7c8b0f.html
data "azurerm_policy_definition_built_in" "dine_activity_logs_to_stream_to_log_analytics_workspace" {
  name = "2465583e-4e78-4c15-b6be-a36cbc7c8b0f"
}

# https://www.azadvertizer.net/azpolicyadvertizer/6c112d4e-5bc7-47ae-a041-ea2d9dccd749.html
data "azurerm_policy_definition_built_in" "deny_deployment_of_classic_resources" {
  name = "6c112d4e-5bc7-47ae-a041-ea2d9dccd749"
}

# https://www.azadvertizer.net/azpolicyadvertizer/06a78e20-9358-41c9-923c-fb736d382a4d.html
data "azurerm_policy_definition_built_in" "audit_vms_that_do_not_use_managed_disks" {
  name = "06a78e20-9358-41c9-923c-fb736d382a4d"
}

# https://www.azadvertizer.net/azpolicyadvertizer/0a914e76-4921-4c19-b460-a2d36003525a.html
data "azurerm_policy_definition_built_in" "audit_resource_location_matches_resource_group_location" {
  name = "0a914e76-4921-4c19-b460-a2d36003525a"
}

# https://www.azadvertizer.net/azpolicyadvertizer/88c0b9da-ce96-4b03-9635-f29a937e2900.html
data "azurerm_policy_definition_built_in" "deny_ip_forwarding_network_interface" {
  name = "88c0b9da-ce96-4b03-9635-f29a937e2900"
}

# https://www.azadvertizer.net/azpolicyadvertizer/404c3081-a854-4457-ae30-26a93ef643f9.html
data "azurerm_policy_definition_built_in" "audit_storage_without_https" {
  name = "404c3081-a854-4457-ae30-26a93ef643f9"
}

# https://www.azadvertizer.net/azpolicyadvertizer/a8eff44f-8c92-45c3-a3fb-9880802d67a7.html
data "azurerm_policy_definition_built_in" "dine_aks_policy_addon" {
  name = "a8eff44f-8c92-45c3-a3fb-9880802d67a7"
}

# https://www.azadvertizer.net/azpolicyadvertizer/25da7dfb-0666-4a15-a8f5-402127efd8bb.html
data "azurerm_policy_definition_built_in" "dine_sql_auditing" {
  name = "25da7dfb-0666-4a15-a8f5-402127efd8bb"
}

# https://www.azadvertizer.net/azpolicyadvertizer/36d49e87-48c4-4f2e-beed-ba4ed02b71f5.html
data "azurerm_policy_definition_built_in" "dine_sql_threat" {
  name = "36d49e87-48c4-4f2e-beed-ba4ed02b71f5"
}

# https://www.azadvertizer.net/azpolicyadvertizer/86a912f6-9a06-4e26-b447-11b16ba8659f.html
data "azurerm_policy_definition_built_in" "dine_sql_encryption" {
  name = "86a912f6-9a06-4e26-b447-11b16ba8659f"
}

# https://www.azadvertizer.net/azpolicyadvertizer/94de2ad3-e0c1-4caf-ad78-5d47bbc83d3d.html
data "azurerm_policy_definition_built_in" "modify_ddos_protection_virtual_network" {
  name = "94de2ad3-e0c1-4caf-ad78-5d47bbc83d3d"
}

# https://www.azadvertizer.net/azpolicyadvertizer/95edb821-ddaf-4404-9732-666045e056b4.html
data "azurerm_policy_definition_built_in" "deny_aks_privileged" {
  name = "95edb821-ddaf-4404-9732-666045e056b4"
}

# https://www.azadvertizer.net/azpolicyadvertizer/1c6e92c9-99f0-4e55-9cf2-0c234dc48f99.html
data "azurerm_policy_definition_built_in" "audit_aks_priv_escalation" {
  name = "1c6e92c9-99f0-4e55-9cf2-0c234dc48f99"
}

# https://www.azadvertizer.net/azpolicyadvertizer/1a5b4dca-0b6f-4cf5-907c-56316bc1bf3d.html
data "azurerm_policy_definition_built_in" "deny_aks_without_https" {
  name = "1a5b4dca-0b6f-4cf5-907c-56316bc1bf3d"
}

# https://www.azadvertizer.net/azpolicyadvertizer/564feb30-bf6a-4854-b4bb-0d2d2d1e6c66.html
data "azurerm_policy_definition_built_in" "audit_app_gw_waf" {
  name = "564feb30-bf6a-4854-b4bb-0d2d2d1e6c66"
}

# https://www.azadvertizer.net/azpolicyadvertizer/98d0b9f8-fd90-49c9-88e2-d3baf3b0dd86.html
data "azurerm_policy_definition_built_in" "dine_vm_backup" {
  name = "98d0b9f8-fd90-49c9-88e2-d3baf3b0dd86"
}

# https://www.azadvertizer.net/azpolicyadvertizer/7bca8353-aa3b-429b-904a-9229c4385837.html
data "azurerm_policy_definition_built_in" "audit_subnet_private" {
  name = "7bca8353-aa3b-429b-904a-9229c4385837"
}

# https://www.azadvertizer.net/azpolicyadvertizer/e56962a6-4747-49cd-b67b-bf8b01975c4c.html
data "azurerm_policy_definition_built_in" "deny_locations" {
  name = "e56962a6-4747-49cd-b67b-bf8b01975c4c"
}

# https://www.azadvertizer.net/azpolicyadvertizer/8e3e61b3-0b32-22d5-4edf-55f87fdb5955.html
data "azurerm_policy_definition_built_in" "dine_log_analytic" {
  name = "8e3e61b3-0b32-22d5-4edf-55f87fdb5955"
}

# https://www.azadvertizer.net/azpolicyadvertizer/78460a36-508a-49a4-b2b2-2f5ec564f4bb.html
data "azurerm_policy_definition_built_in" "denyaction_resource_deletion" {
  name = "78460a36-508a-49a4-b2b2-2f5ec564f4bb"
}

# https://www.azadvertizer.net/azpolicyadvertizer/e765b5de-1225-4ba3-bd56-1ac6695af988.html
data "azurerm_policy_definition_built_in" "deny_location_resource_groups" {
  name = "e765b5de-1225-4ba3-bd56-1ac6695af988"
}

# https://www.azadvertizer.net/azpolicyadvertizer/83a86a26-fd1f-447c-b59d-e51f44264114.html
data "azurerm_policy_definition_built_in" "deny_public_ip_address_on_nic" {
  name = "83a86a26-fd1f-447c-b59d-e51f44264114"
}

# https://www.azadvertizer.net/azpolicyadvertizer/6c112d4e-5bc7-47ae-a041-ea2d9dccd749.html
data "azurerm_policy_definition_built_in" "deny_hybrid_networking" {
  name = "6c112d4e-5bc7-47ae-a041-ea2d9dccd749"
}

